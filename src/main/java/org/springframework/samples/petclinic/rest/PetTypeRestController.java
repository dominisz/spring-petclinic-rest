...
public class PetTypeRestController {
	...
	public ResponseEntity<PetType> addPetType(@RequestBody @Valid PetType petType, BindingResult bindingResult, UriComponentsBuilder ucBuilder){
        if(bindingResult.hasErrors() || (petType == null)){
            return createPetTypeErrorResponses(bindingResult);
		}
		this.petTypeService.savePetType(petType);
		return new ResponseEntity<PetType>(petType, createLocationHeader(ucBuilder, petType), HttpStatus.CREATED);
	}
    ...
    private HttpHeaders createLocationHeader(UriComponentsBuilder ucBuilder, PetType petType){
        HttpHeaders headers = new HttpHeaders();
        headers.setLocation(ucBuilder.path("/api/pettypes/{id}").buildAndExpand(petType.getId()).toUri());
        return headers;
    }

	private ResponseEntity<PetType> createPetTypeErrorResponses(BindingResult bindingResult){
